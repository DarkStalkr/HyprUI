...
Singleton {
    ...
    FileView {
        ...
        path: `${Quickshell.env("XDG_CONFIG_HOME") || `${Quickshell.env("HOME")}/.config`}/hyprui/shell.json`
        watchChanges: true
        onFileChanged: {
        ...
        onSaveFailed: err => console.error("Failed to save config:", FileViewError.toString(err))
